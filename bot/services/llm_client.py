...
class LLMClient:
        ...
        async with LMSClient() as backend:
            if tool_name == "get_items":
                ...
                resp = await backend._client.get(
                    f"{backend.base_url}/learners/",
                    headers={"Authorization": f"Bearer {backend.api_key}"},
                )
                return resp.json() if resp.status_code == 200 else []
            elif tool_name == "get_scores":
                resp = await backend._client.get(
                    f"{backend.base_url}/analytics/scores/",
                ...
                resp = await backend._client.get(
                    f"{backend.base_url}/analytics/timeline/",
                    params={"lab": args.get("lab")},
                    headers={"Authorization": f"Bearer {backend.api_key}"},
                )
                return resp.json() if resp.status_code == 200 else []
                ...
                resp = await backend._client.get(
                    f"{backend.base_url}/analytics/groups/",
                    params={"lab": args.get("lab")},
                    headers={"Authorization": f"Bearer {backend.api_key}"},
                )
                return resp.json() if resp.status_code == 200 else []
                ...
                resp = await backend._client.get(
                    f"{backend.base_url}/analytics/top-learners/",
                ...
                resp = await backend._client.get(
                    f"{backend.base_url}/analytics/completion-rate/",
                    params={"lab": args.get("lab")},
                    headers={"Authorization": f"Bearer {backend.api_key}"},
                )
                return resp.json() if resp.status_code == 200 else {}
                ...
                resp = await backend._client.post(
                    f"{backend.base_url}/pipeline/sync",
                    headers={"Authorization": f"Bearer {backend.api_key}"},
                )
    ...
    async def chat_with_tools(
        self, messages: List[Dict[str, Any]], max_turns: int = 3
    ) -> str:
        ...
        for turn in range(max_turns):
            ...
            try:
                ...
                print(
                    f"[llm] Error: {type(e).__name__}: {str(e)[:100]}", file=sys.stderr
                )
                return (
                    f"Sorry, I couldn't reach the AI service. Please try again later."
                )

            choice = data.get("choices", [{}])[0].get("message", {})
            if (
                "content" in choice
                and choice.get("content")
            ...
            if tool_calls:
                ...
                for tc in tool_calls:
                    ...
                    print(
                        f"[tool] Result: {len(result) if isinstance(result, (list, dict)) else 'ok'} items",
                        file=sys.stderr,
                    )
                    messages.append(
                        {
